...
- **Acesso Restrito:** Implementação de login obrigatório via Supabase Auth.
- **Whitelist de E-mails:** Apenas e-mails autorizados (`nudoze@proton.me` e `mcdouglasf4phantomii@gmail.com`) possuem permissão de entrada.
- **Recuperação de Senha:** Fluxo completo de solicitação de recuperação e redefinição de senha via e-mail.
- **Proteção de Rotas:** Bloqueio de acesso direto ao Dashboard sem uma sessão ativa.
- **Segurança de Variáveis:** Credenciais sensíveis movidas para variáveis de ambiente (`.env`).

## 5. Requisitos Não Funcionais
- **Performance:** Carregamento rápido de componentes e transições suaves.
- **Segurança:** Implementação obrigatória de RLS no Supabase para proteção de dados e autenticação via JWT.
...
- **Backend/Banco de Dados:** Supabase (Auth + PostgreSQL + RLS).
- **Ícones:** Lucide React.
- **Gráficos:** Recharts.
...
- Expansão da Whitelist para novos sócios ou administradores.
